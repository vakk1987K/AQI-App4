...
import { AlertTriangle, ShieldCheck, Sparkles, Smartphone, ArrowRight, Heart, Landmark } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-sky-500 selection:text-white flex flex-col">
      {/* Top Banner addressing Google Play rejection & closed testing status */}
      <div className="bg-amber-500/10 border-b border-amber-500/25 px-4 py-2 text-xs text-amber-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className="p-1 rounded bg-amber-500/20 text-amber-400 shrink-0">
            <AlertTriangle className="w-3.5 h-3.5" />
          </span>
          <span>
            <strong>Google Play Console Notice:</strong> Closed test release for <code className="text-amber-300 font-mono">app.vercel.aqi_app3.twa</code> is inactive (App rejected / 0 of 12 testers opted in).
          </span>
        </div>
        <button
          onClick={() => setIsPlayConsoleOpen(true)}
          id="play-console-banner-btn"
          className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold transition text-xs shrink-0 self-start sm:self-auto"
        >
          <span>Open 12-Tester & Policy Hub</span>
